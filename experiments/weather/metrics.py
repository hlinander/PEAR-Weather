...
def anomaly_correlation_coefficient_dh_on_dh(model, dataloader_dh, device_id):
    ...
    for idx, batch_dh in tqdm.tqdm(enumerate(dataloader_dh)):
        ...
        if not initialized:
            initialized = True
            logit_surface_squared = weights * out_surface**2
            target_surface_squared = weights * target_surface**2
            logit_upper_squared = weights * out_upper**2
            target_upper_squared = weights * target_upper**2

            nominator_surface = weights * out_surface * target_surface
            nominator_upper = weights * out_upper * target_upper
        else:
            logit_surface_squared += weights * out_surface**2
            target_surface_squared += weights * target_surface**2
            logit_upper_squared += weights * out_upper**2
...
def rmse_hp(model, dataloader, device_id):
    ...
    for idx, batch in tqdm.tqdm(enumerate(dataloader)):
        ...
        if not initialized:
            initialized = True
            rmse_surface_batches = torch.sqrt(((out_surface - target_surface) ** 2))
            rmse_upper_batches = torch.sqrt(((out_upper - target_upper) ** 2))
...
def rmse_dh_on_dh(model, dataloader_dh, device_id, weighted=True):
    ...
    for idx, batch_dh in tqdm.tqdm(enumerate(dataloader_dh)):
        ...
        if not initialized:
            ...
            rmse_upper_batches = weights * torch.sqrt(((out_upper - target_upper) ** 2))
            rmse_surface = rmse_surface_batches.sum(dim=0) / n_samples
            rmse_upper = rmse_upper_batches.sum(dim=0) / n_samples
        else:
            rmse_surface_batches = weights * torch.sqrt(
